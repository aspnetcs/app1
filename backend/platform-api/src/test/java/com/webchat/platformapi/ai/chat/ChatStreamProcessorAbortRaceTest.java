...
class ChatStreamProcessorAbortRaceTest {

    @Test
    void streamWhenAbortArrivesAfterLastDeltaDoesNotSendChatDone() throws Exception {
        ...
        String requestId = "req-race";
        String traceId = "trace-race";
        ...
        doAnswer(invocation -> {
            streamContextRegistry.markAborted(userId, requestId, "user_cancel");
            streamContextRegistry.markTerminalSentIfAbsent(userId, requestId);
            return null;
        }).when(owner).finalizeReservedQuotaAfterStream(eq(userId), eq(10L), eq(11L), eq("openai"),
                eq("gpt-5.4-mini"), eq(0), eq(0), eq(0), any(Long.class), eq(true), eq(requestId), eq(false), eq(true));
        doNothing().when(owner).sendDeltaOrThrow(userId, requestId, traceId, requestBody, "hello");

        processor.stream(owner, userId, requestId, traceId, requestBody, "127.0.0.1", "JUnit");

        verify(owner).sendDeltaOrThrow(userId, requestId, traceId, requestBody, "hello");
        verify(ws, never()).sendToUser(eq(userId), eq("chat.done"), any());
        verify(owner).audit(eq(userId), eq(requestId), eq("gpt-5.4-mini"), eq(11L), eq("openai"), eq(22L),
                eq("aborted"), eq("user_cancel"), any(), eq("127.0.0.1"), eq("JUnit"), eq(0), eq(0), eq(0));
    }
}
