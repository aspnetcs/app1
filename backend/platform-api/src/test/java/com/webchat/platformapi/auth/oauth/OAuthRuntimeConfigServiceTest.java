...
class OAuthRuntimeConfigServiceTest {
    ...
    void setUp() {
        ...
        properties.setAutoLinkByEmail(true);
    ...
    void currentConfigNormalizesConfiguredProviderKeysBeforeApplyingOverrides() {
        when(valueOperations.get("oauth:user:config")).thenReturn("""
                {"enabled":true,"autoLinkByEmail":false,"providers":{"GITHUB":{"enabled":false,"allowUserLogin":false}}}
                """);
        ...
        assertThat(config.getProvider("github").allowUserLogin()).isFalse();
    }
    ...
    void saveAcceptsMixedCaseOverrideKeysAndPersistsNormalizedKeys() {
        ...
        verify(valueOperations).set(eq("oauth:user:config"), jsonCaptor.capture());
